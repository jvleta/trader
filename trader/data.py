...
def get_spot_price(symbol: str) -> float:
    try:
        ...
        raise RuntimeError(f"Failed to retrieve spot price for symbol: {symbol}") from exc
...
def get_dividend_yield(symbol: str, price: float | None = None) -> float:
    try:
        ...
        dividends = getattr(response, "data", None) or getattr(response, "results", None)
        ...
        raise RuntimeError(f"Failed to retrieve dividend yield for symbol: {symbol}") from exc
    ...
    try:
        ...
        for entry in reversed(records):
            ...
            if value is None and isinstance(entry, dict):
                value = entry.get("year_10") or entry.get("year10") or entry.get("ten_year")
            if value is None:
                continue
            try:
                rate = float(value)
...
def fetch_option_chain(symbol: str, filter_expiries: list | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to retrieve option chain for symbol: {symbol}") from exc
    ...
    price_candidates = [
        col for col in ("last_trade_price", "close", "mark", "theoretical_price", "prev_close")
        if col in df.columns
    ]
    if price_candidates:
        df.loc[:, "market_price"] = (
            df[price_candidates].apply(pd.to_numeric, errors="coerce").bfill(axis=1).iloc[:, 0]
        )
    else:
        df["market_price"] = np.nan

    keep_cols = [
        "symbol", "option_type", "expiration", "strike", "market_price",
        "bid", "ask", "volume", "open_interest", "implied_volatility", "T_years",
    ]
